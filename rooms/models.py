...
class Room(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["property", "is_active", "operational_status"], name="idx_rm_prop_act_status"),
            models.Index(fields=["room_type", "is_active"], name="idx_rm_type_active"),
        ]
    ...
    def calculate_price(self, check_in, check_out):
        ...
        Uses prefetched cache on self.rates when available.
        """
        from datetime import timedelta

        # Use prefetched cache if available, else filter via DB
        if hasattr(self, "_prefetched_objects_cache") and "rates" in self._prefetched_objects_cache:
            rates = [r for r in self.rates.all() if r.start_date < check_out and r.end_date > check_in]
        else:
...
class Booking(models.Model):
    ...
    coupon = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        related_name="bookings",
        help_text="Loyalty coupon or promo code applied to this booking.",
    )
    discount_amount = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=Decimal('0.00'),
        help_text="Discount amount deducted from total price.",
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["room", "check_in", "check_out", "status"], name="idx_bk_rm_dates_status"),
            models.Index(fields=["check_in", "check_out", "status"], name="idx_bk_dates_status"),
            models.Index(fields=["user", "status", "-created_at"], name="idx_bk_usr_st_created"),
        ]
    ...
    def compute_tax(self):
        ...
        try:
            ...
            cfg = cache.get_or_set(f"ttr_tax_config_prop_{prop.id}", lambda: prop.tax_config, timeout=86400)
            taxable_base = max(Decimal('0.00'), self.total_price - (self.discount_amount or Decimal('0.00')))
            nightly_rate = taxable_base / self.num_nights if self.num_nights else taxable_base
            rate = cfg.gst_rate_for(nightly_rate)
            self.tax_amount = (taxable_base * rate / Decimal('100')).quantize(Decimal('0.01'))
        except Exception:
    ...
    def payable_amount(self):
        """Final payable amount in INR after applying discount and taxes."""
        from decimal import Decimal
        base = max(Decimal('0.00'), self.total_price - (self.discount_amount or Decimal('0.00')))
        return (base + (self.tax_amount or Decimal('0.00'))).quantize(Decimal('0.01'))
